...
public class AutoAlignment {
    private static final HashMap<Alignable, BiFunction<Double, Double, Rotation2d>> positionAngleMap =
            new HashMap<>(
                    Map.of(
                            // Alignable.AMP, (x, y) -> new Rotation2d(Math.PI / 2),
                            // Alignable.SPEAKER, AutoAlignment::calcRobotRotationFaceSpeaker
                    ));
    private static final HashMap<Alignable, BiFunction<Double, Translation3d, VelocityVector>>
            positionYawMap =
            new HashMap<>(
                    Map.of(
                            // Alignable.AMP, (aDouble, translation3d) -> new VelocityVector(10.5,
                            // Rotation2d.fromDegrees(Ramp.encoderToAngle(Constants.AMP_RAMP_ENC_VALUE))),
     ...
     * @return a {@link Rotation2d} from the ground to the ramp representing the desired angle for
     *     shooting
     * @see #calcRampAngle(double, double, double, double)
     */

    /**
     * Calculates the desired ramp angle for shooting into the speaker given an initial velocity, and
     * the robots position
     *
     ...
     * @param pose3d a 3 dimensional pose representing the robots center x position, center y
     *     position, and z distance from the ground (include base of robot)
     * @return a {@link Rotation2d} from the ground to the ramp representing the desired angle for
     *     shooting
     */
    /**
     * Uses basic trig to calculate the desired angle to face the speaker given distance in both x and
     * y directions
     *
    ...
    private static Rotation2d calcRobotRotationFaceSpeaker(double x, double y) {
        return new Rotation2d(RobotContainer.isRedAlliance() ? 0 : Math.PI)
                .plus(new Rotation2d(Math.atan(y / x)));
    }

    public static double calcTurnSpeed(
            Alignable alignable, Rotation2d currentAngle, double x, double y, PIDController controller) {
        ...
        double clamp =
                MathUtil.clamp(
                        controller.calculate(currentAngle.getDegrees(), targetAngle),
                        -1 * Constants.MAX_AUTO_ALIGN_SPEED,
                        Constants.MAX_AUTO_ALIGN_SPEED);
    ...
    public static double calcTurnSpeed(
            Alignable alignable, Pose2d currentPos, PIDController controller) {
        return calcTurnSpeed(
                alignable, currentPos.getRotation(), currentPos.getX(), currentPos.getY(), controller);
    }

    public static double angleFromTarget(
            Alignable alignable, Rotation2d currentAngle, double x, double y) {
    ...
    public static double angleFromTarget(Alignable alignable, Pose2d currentPose) {
        return angleFromTarget(
                alignable, currentPose.getRotation(), currentPose.getX(), currentPose.getY());
    }
     ...
     */
    public static VelocityVector getYaw(
            Alignable alignable, double x, double y, double z, double drivetrainVelX) {
    ...
    /**
     * @param alignable what we are aligning to
     * @param pose3d ramp position in 3d space (use robot position plus height of ramp, DO NOT
     *     manually account for RAMP_X_OFFSET)
     * @param drivetrainVelX velocity the robo is moving in the x direction
     * @return the desired {@link Rotation2d} of the ramp from the ground
     */
    public static VelocityVector getYaw(
            Alignable alignable, Translation3d pose3d, double drivetrainVelX) {
        return getYaw(alignable, pose3d.getX(), pose3d.getY(), pose3d.getZ(), drivetrainVelX);
    }

    private static boolean isInvalidYawFunction(
            BiFunction<Double, Translation3d, VelocityVector> function) {
        if (function == null) {
